...
/**
 * @Description 从HBase表读写数据Client封装类，方便使用
 * @Date 11:28 2022/3/21
 * @Param
 * @return
 * */
object HBaseTools {
    ...
    def read(spark: SparkSession, zks: String, port: String, table: String, family: String, fields: Seq[String]): DataFrame = {
        ...
        conf.set(HBASE_ZK_QUORUM_KEY, zks)
        conf.set(HBASE_ZK_PORT_KEY, port)
        // 2. 设置读HBase表的名称
    ...
    def write(dataframe: DataFrame, zks: String, port: String, table: String, family: String, rowKeyColumn: String): Unit = {
        ...
        putsRDD.saveAsNewAPIHadoopFile(
            s"/hbase/hfile_tmp/$table-" + System.currentTimeMillis(),
            classOf[ImmutableBytesWritable], //
            classOf[Put], //
