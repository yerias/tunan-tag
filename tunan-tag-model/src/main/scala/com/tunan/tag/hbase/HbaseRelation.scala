...
class HbaseRelation(context: SQLContext,
    ...
    override def buildScan(): RDD[Row] = {
        ...
        params.keys.foreach(key => {
            println(s"key: ${key}, value: ${params(key)}")
        })
        ...
        if (StringUtils.isNotBlank(filterConditions)) {
            //构建FilterList对象
        ...
        conf.set(
            ...
            new String(org.apache.commons.codec.binary.Base64.encodeBase64(ProtobufUtil.toScan(scan).toByteArray))
        )
